...
beforeEach(() => {
  todoItem1 = new TodoItem(1, '할 일1', new Date());
  todoItem2 = new TodoItem(2, '할 일2', new Date());
  todoItem3 = new TodoItem(3, '할 일3', new Date());
  todoItem4 = new TodoItem(4, '할 일4', new Date());
  todoItem5 = new TodoItem(5, '할 일5', new Date());
...
describe("할 일 목록에서 할 일을 추가할 수 있다.", () => {
  test("5개의 할 일이 있는데, 1개의 할 일 추가하기", () => {
...
describe("생성한 할 일들 중에서 오늘 할 일, 지난 할 일 구분하기", () => {
  ...
  test("5개의 할 일이 있는데, id가 2번만 지난 할 일이다.", () => {
    jest.spyOn(todoItem2, "equalsDayOfCreatedAt").mockImplementation(() => false);
...
describe("오늘 할 일 중 전체/완료/미완료 구분하기", () => {
