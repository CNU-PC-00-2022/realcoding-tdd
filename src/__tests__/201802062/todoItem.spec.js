...
describe("할 일을 만들 수 있다.", () => {
  test("todo item 생성하기", () => {
    const todoItem = new TodoItem(1, '오늘은 술 먹는날.', Date.now(), false)
...
describe("할 일을 완료로 생성할 수 있다.", () => {
  test("todo item 완료로 바꾸기", () => {
    const todoItem = new TodoItem(1, '오늘은 술 먹는날.', Date.now(), true);
    // todoItem.setComplete();
    expect(todoItem.completed).toBeTruthy();
  });

  test("todo item 미완료로 바꾸기", () => {
    const todoItem = new TodoItem(1, '오늘은 술 먹는날.', Date.now());
...
describe("할 일에 날짜가 들어간다.", () => {
  test("todo item이 오늘만들었으면, isToday가 true이다.", () => {
  ...
  test("todo item이 어제만들었으면, isToday가 false이다.", () => {
