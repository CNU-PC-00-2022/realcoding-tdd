...
describe("할 일에 날짜가 들어간다.", () => {
  test("todo item이 오늘 만들었으면, isToday가 true이다.", () => {
    ...
    expect(todoItem.equalsDayofCreadtedAt(targetDate)).toBeTruthy();
  });
  test("todo item이 어제 만들었으면, isToday가 false이다.", () => {
    ...
    expect(todoItem.equalsDayofCreadtedAt(targetDate)).toBeFalsy();
  });
