...
describe("오늘 할 일 중 완료/미완료 구분하기", () => {
  test("5개의 할 일이 있는데, 2번, 3번만 완료다.", () => {
    ...
    expect(
      todoList.equalsDayAndCompletedItems.some((todoItem) => todoItem.id === 2)
    ).toBeTruthy();
    expect(
      todoList.equalsDayAndCompletedItems.some((todoItem) => todoItem.id === 3)
    ).toBeTruthy();
  });
  ...
  test("5개의 할 일이 있는데, 2번, 4번만 미완료다.", () => {
    ...
    expect(
      todoList.equalsDayAndNotCompletedItems.some(
        (todoItem) => todoItem.id === 2
      )
    ).toBeTruthy();
    expect(
      todoList.equalsDayAndNotCompletedItems.some(
...
describe("지난 할 일 중 완료/미완료 구분하기", () => {
  test("5개의 지난 할 일이 있는데, 2번, 3번만 완료다.", () => {
    ...
    expect(
      todoList.notEqualsDayAndCompletedItems.some(
        (todoItem) => todoItem.id === 2
      )
    ).toBeTruthy();
    expect(
      todoList.notEqualsDayAndCompletedItems.some(
        (todoItem) => todoItem.id === 3
      )
    ).toBeTruthy();
  });
  ...
  test("5개의 지난 할 일이 있는데, 2번, 4번만 미완료다.", () => {
    ...
    expect(
      todoList.notEqualsDayAndNotCompletedItems.some(
        (todoItem) => todoItem.id === 2
      )
    ).toBeTruthy();
    expect(
