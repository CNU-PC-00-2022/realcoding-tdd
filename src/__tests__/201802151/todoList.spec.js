...
describe("할 일 목록에서 추가를 수행할 수 있다.", () => {
...
describe("생성한 할 일들 중에서 오늘 할 일, 지난 할 일 구분하기", () => {
  test("5개의 할 일이 있는데, 2번 뺴고 오늘 만들 일 이다.", () => {
    ...
    expect(
      todoList.equalsDayItems.some((item) => {
        return item.id === 2;
      })
    ).toBeFalsy();
  });
  ...
  test("5개의 할 일이 있는데, id 2번만 지난 할 일이다.", () => {
    ...
    expect(
      todoList.notEqualsDayItems.some((item) => {
        return item.id === 2;
      })
    ).toBeTruthy();
  });
